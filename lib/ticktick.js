...
const TOKEN_RETRY_MS = 60_000; // retry loading token every 60s if null

/** @returns {string|null} */
function getToken() {
  if (_token) return _token;
  // Retry periodically in case token appears after startup
  if (Date.now() - _tokenCheckedAt > TOKEN_RETRY_MS) {
    _token = loadAccessToken();
...
export async function fetchProjects(forceRefresh = false) {
  ...
  let projects;
  try {
    projects = await ticktickFetch("GET", "/project");
  } catch (err) {
    _projectCache = null; // invalidate on error so next call retries
    throw err;
  }
