...
import { LOG_PREFIX, isDuplicateMemory, markMemoryAdded } from "./client.js";
...
export async function findTasks({ status, priority, project } = {}) {
  const [tasks, updates] = await Promise.all([
    searchTextMemories("task", 30, { filter: { _type: "task" } }).catch(() => []),
    searchTextMemories("TASK_COMPLETED task done", 30, { filter: { _type: "task_update" } }).catch(() => []),
  ]);
