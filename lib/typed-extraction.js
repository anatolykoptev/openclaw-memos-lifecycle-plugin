...
export async function extractAllTypedMemories(conversationText) {
  ...
  const results = await Promise.allSettled(
    relevantTypes.map((type) => extractTypedMemories(type, conversationText)),
  );

  const allMemories = [];
  for (let i = 0; i < results.length; i++) {
    const { status, value, reason } = results[i];
    if (status === "fulfilled" && value?.length > 0) {
      console.log(LOG_PREFIX, `Extracted ${value.length} ${relevantTypes[i]} memories`);
      allMemories.push(...value);
    } else if (status === "rejected") {
      console.warn(LOG_PREFIX, `Failed to extract ${relevantTypes[i]}:`, reason?.message);
    }
  }
