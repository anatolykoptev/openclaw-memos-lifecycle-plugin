...
export function rewriteQuery(prompt, isPostCompaction = false) {
  ...
  // Project and task queries (v2.2)
  if (/\b(проект|project|задач|task|todo|делаем|plan)\b/i.test(lower)) queryParts.push("PROJECT STATUS active PRIORITY");
  if (/\b(статус|status|что нов|update|progress)\b/i.test(lower)) queryParts.push("SYSTEM_STATUS current health");
  if (/\b(кто я|who am|обо мне|about me|мой профиль)\b/i.test(lower)) queryParts.push("USER_PROFILE identity preferences");
