...
FORMAT each task as:
TASK: [name] | STATUS: pending | PRIORITY: [P0/P1/P2]
...
- Include context if provided (deadline, project)
- Return JSON array of task strings. Empty array [] if nothing.
...
export function detectRelevantTypes(conversationText) {
  ...
  if (
    /\b(нужно|надо|хочу сделать|plan to|need to|should|want to|todo|задача|сделай)\b/i.test(lower)
  ) {
