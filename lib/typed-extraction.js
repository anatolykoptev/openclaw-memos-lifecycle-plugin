 ...
 */
import { callApi, MEMOS_USER_ID, MEMOS_CUBE_ID, Timeouts, LOG_PREFIX } from "./client.js";
import {
  MemoryTypes,
...
export async function extractTypedMemories(type, conversationText) {
  ...
  try {
    const result = await callApi(
      ...
      { query: prompt, user_id: MEMOS_USER_ID, mem_cube_id: MEMOS_CUBE_ID, enable_memory: false },
    ...
    try {
      ...
      try {
        ...
        console.warn(LOG_PREFIX, `Could not parse ${type} extraction JSON`);
        return [];
      }
    }
    ...
    return parsed
      .filter((item) => {
        ...
        if (typeof item === "object" && item.content) return item.content.length > 10;
        return false;
      })
      .map((item) => {
        const content = typeof item === "string" ? item : item.content;
        const itemType = typeof item === "object" && item.type ? item.type : type;
        return {
          content,
